...
class ATRSettings:
    def __init__(self, manufacturer: str=None, version: str=None, parameters: dict=None) -> None:
        ...
        self.parameters = CustomArray1DDcsLongString(len(parameters)) if parameters is not None else CustomArray1DDcsLongString(0)
    ...
    def as_dict(self) -> dict:
        ...
        """
        return {"manufacturer": self.manufacturer.Get(), "version": self.version.Get(), 
                "parameters": {self.parameters[i].Get().split("=")[0].strip("-"): self.parameters[i].Get().split("=")[1] for i in range(self.parameters.GetSize())}}
