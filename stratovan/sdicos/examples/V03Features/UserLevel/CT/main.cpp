...
void ParentChanges()
{
	...
	*/
	{
		SDICOS::ScanCommon& scanCommon = ct;
			...
			*/
			{
				...
				ct.SetTipLibraryName("EXAMLE LIBRARY NAME");
				
				//Check if the library name is set, if it is get it
				if(ct.HasTipLibraryName())
				{
					SDICOS::DcsShortText strLibraryName;
					strLibraryName = ct.GetTipLibraryName();
				}

				//Delete the library name
				ct.DeleteTipLibraryName();
			}
			...
			*/
			{
				...
				ct.SetTipImageIdentifier("EXAMPLE IMAGE IDENTIFIER");

				//Check if the image identifier is set, if it is get it
				if(ct.HasTipImageIdentifier())
				{
					SDICOS::DcsShortText strImageIdentifier;
					strImageIdentifier = ct.GetTipImageIdentifier();
				}

				//Delete the image identifier
				ct.DeleteTipImageIdentifier();
			}
			...
			*/
			{
				...
				ct.SetTipLibraryDate(dtLibraryDate);

				//Check if the library date is set, if it is get it
				if(ct.HasTipLibraryDate())
				{
					dtLibraryDate = ct.GetTipLibraryDate();
				}

				//Delete the library date
				ct.DeleteTipLibraryDate();
			}
		}
	}
		...
		*/
	{
		...
		ct.EnableObjectOfInspectionOwner();

		ct.IsObjectOfInspectionOwnerEnabled();
		...
		ct.DisableObjectOfInspectionOwner();

		// Get Object of Inspection Owner
		SDICOS::ObjectOfInspectionOwnerUser& OOIOwner = ct;
		...
		// IMPORTANT: Do not confuse with ct.SetGender() 
	...
	//
	{
		...
		SDICOS::ItineraryUser& itinerary = ct;
	...
	// XRayEquipmentUser Features
	{
		/*  X-Ray Equipment User Level API 
			...
			*/
		SDICOS::XRayEquipmentUser& xRayEquipment = ct;
	...
	//
	{
		SDICOS::FrameOfReferenceUser &frameOfReference = ct;
	...
	//
	{
		SDICOS::AcquisitionContextUser &acquisitionContext = ct;
...
void ImageChanges()
{
	...
	*/
	{
		//'Set' function
		ct.SetImageOrigin(SDICOS::ImageCommon::enumMachine);
		ct.SetImageOrigin(SDICOS::ImageCommon::enumAugmented);
		ct.SetImageOrigin(SDICOS::ImageCommon::enumSynthetic);
	...
	*/
	{
		//'Set' function
		ct.SetCoordinateSystem(SDICOS::ImageCommon::enumLHS);
		ct.SetCoordinateSystem(SDICOS::ImageCommon::enumRHS);

		//'Get' function
		ct.GetCoordinateSystem();
	}

	/* SDI/CDI Data Type - New SOP Class UIDs for CT scans to indicate if the data is standard, raw, or corrected
	*
	* SDI - Sensor Data Interface indicates that the CT image is raw data
	* CDI - Corrected/Constructed Data Interface indicates the CT image is corrected data
	* Standard - Standard indicate sthe CT image is standard data, this is the default
	*/
	{
		//Set SDI
		ct.SetSDI();
		//Check if a ct object is using SDI data
		ct.IsSDI();

		//Set CDI
		ct.SetCDI();
		//Check if a ct object is using CDI data
		ct.IsCDI();

		//Set Standard, this is the default
		ct.SetStandard();
		//Check if a ct object is using Standard data
		ct.IsStandard();
	}
}
